...
| [dart-ffi-code-assets](skills/dart-ffi-code-assets/SKILL.md) | Guide agents to compile C/C++ code into Code Assets using the Native Assets feature (via build and link hooks using `package:hooks` and `package:native_toolchain_c`). Covers local dynamic compilation, linker tree-shaking via `record_use` tracking, and alternative precompiled library download workflows with cryptographic hash validation. | Compile C code into Code Assets using native assets hooks. |
